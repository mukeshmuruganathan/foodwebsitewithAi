...
exports.forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    const frontendURL = process.env.FRONTEND_URL || process.env.FRONTEND_url || "http://localhost:5174";
    const cleanUrl = frontendURL.replace(/"/g, "").replace(/\/$/, "");
    const resetURL = `${cleanUrl}/users/resetPassword/${resetToken}`;
...
// Make currently logged in user an admin (for testing admin UI)
exports.makeMeAdmin = catchAsyncErrors(async (req, res, next) => {
  const user = await User.findById(req.user.id);
  if (!user) {
